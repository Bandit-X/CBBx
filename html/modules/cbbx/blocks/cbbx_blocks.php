...
if(defined('CBBX_BLOCK_DEFINED')) return;
define('CBBX_BLOCK_DEFINED',true);

function b_cbbx_array_filter($var){
	return $var > 0;
}
...
function b_cbbx_show($options)
{
    ...
    $extra_criteria = "";
	if(!empty($options[2])) {
		$time_criteria = time() - cbbx_getSinceTime($options[2]);
    ...
    if (!empty($options[6])) {
        $allowedforums = array_filter(array_slice($options, 6), "b_cbbx_array_filter"); // get allowed forums
    ...
    $query = 'SELECT'.
    		...
    		'	FROM ' . $db->prefix('cbbx_posts') . ' AS p '.
    		'	LEFT JOIN ' . $db->prefix('cbbx_topics') . ' AS t ON t.topic_last_post_id=p.post_id'.
    		'	LEFT JOIN ' . $db->prefix('cbbx_forums') . ' AS f ON f.forum_id=t.forum_id'.
    		'	WHERE 1=1 ' .
    			$forum_criteria .
    			$approve_criteria .
    			$extra_criteria .
    ...
    if (!$result) {
	    cbbx_message("cbbx block query error: ".$query);
        return false;
    }
	...
	$author_name = cbbx_getUnameFromIds(array_keys($author), $cbbxConfig['show_realname'], true);

    foreach ($rows as $arr) {
        $topic_page_jump = '';
        if ($arr['allow_subject_prefix']) {
            $subjectpres = explode(',', $cbbxConfig['subject_prefix']);
        ...
        $topic['time'] = cbbx_formatTimestamp($arr['post_time']);
...
function b_cbbx_topic_show($options)
{
    global $xoopsConfig;
    global $access_forums;
    ...
    $query = 'SELECT'.
    		...
    		'	f.forum_name, f.allow_subject_prefix'.
    		'	FROM ' . $db->prefix('cbbx_topics') . ' AS t '.
    		'	LEFT JOIN ' . $db->prefix('cbbx_forums') . ' AS f ON f.forum_id=t.forum_id'.
    		'	WHERE 1=1 ' .
    			$forum_criteria .
    			$approve_criteria .
    ...
    foreach ($rows as $arr) {
        ...
        $topic['replies'] = $arr['topic_replies'];
        $topic['views'] = $arr['topic_views'];
        $topic['time'] = cbbx_formatTimestamp($arr['topic_time']);
...
function b_cbbx_post_show($options)
{
    global $xoopsConfig;
    global $access_forums;
    ...
    switch ($options[0]) {
	    case "text":
		    if(!empty($cbbxConfig['enable_karma']))
				$extra_criteria .= " AND p.post_karma = 0";
		    if(!empty($cbbxConfig['allow_require_reply']))
    ...
    $query .= '	f.forum_id, f.forum_name, f.allow_subject_prefix'.
    		'	FROM ' . $db->prefix('cbbx_posts') . ' AS p '.
    		'	LEFT JOIN ' . $db->prefix('cbbx_forums') . ' AS f ON f.forum_id=p.forum_id';
    if($options[0]=="text"){
    	$query .= '	LEFT JOIN ' . $db->prefix('cbbx_posts_text') . ' AS pt ON pt.post_id=p.post_id';
	}
    $query .= '	WHERE 1=1 ' .
    			$forum_criteria .
    			$approve_criteria .
    ...
    foreach ($rows as $arr) {
        ...
        //$topic['jump_post'] = "<a href='" . XOOPS_URL . "/modules/cbbx/viewtopic.php?post_id=" . $arr['post_id'] ."#forumpost" . $arr['post_id'] . "'>" . $last_post_icon . "</a>";
        $topic['forum_id'] = $arr['forum_id'];
        $topic['forum_name'] = $myts->htmlSpecialChars($arr['forum_name']);
        //$topic['id'] = $arr['topic_id'];
        ...
        if($options[0]=="text"){
        	...
        	if(!empty($options[5])){
	    		$post_text = xoops_substr(cbbx_html2text($post_text), 0, $options[5]);
    		}
...
function b_cbbx_author_show($options)
{
    global $xoopsConfig;
    global $access_forums;
    ...
    if (!empty($options[5])) {
        $allowedforums = array_filter(array_slice($options, 5), "b_cbbx_array_filter"); // get allowed forums
        $allowed_forums = array_intersect($allowedforums, $access_forums);
    }else{
        $allowed_forums = $access_forums;
    }
	...
	$author_name = cbbx_getUnameFromIds(array_keys($author), $cbbxConfig['show_realname']);
...
function b_cbbx_edit($options)
{
    $form  = _MB_CBBX_CRITERIA."<select name='options[0]'>";
    $form .= "<option value='time'";
    if($options[0]=="time") $form .= " selected='selected' ";
    $form .= ">"._MB_CBBX_CRITERIA_TIME."</option>";
    $form .= "</select>";
    $form .= "<br />" . _MB_CBBX_DISPLAY."<input type='text' name='options[1]' value='" . $options[1] . "' />";
    $form .= "<br />" . _MB_CBBX_TIME."<input type='text' name='options[2]' value='" . $options[2] . "' />";
    $form .= "<br />&nbsp;&nbsp;&nbsp;&nbsp;<small>" . _MB_CBBX_TIME_DESC. "</small>";
    $form .= "<br />" . _MB_CBBX_DISPLAYMODE. "<input type='radio' name='options[3]' value='0'";
    if ($options[3] == 0) {
        $form .= " checked='checked'";
    }
    $form .= " />&nbsp;" . _MB_CBBX_DISPLAYMODE_FULL . "<input type='radio' name='options[3]' value='1'";
    if ($options[3] == 1) {
        $form .= " checked='checked'";
    }
    $form .= " />&nbsp;" . _MB_CBBX_DISPLAYMODE_COMPACT . "<input type='radio' name='options[3]' value='2'";
    if ($options[3] == 2) {
        $form .= " checked='checked'";
    }
    $form .= " />&nbsp;" . _MB_CBBX_DISPLAYMODE_LITE;

    $form .= "<br />" . _MB_CBBX_INDEXNAV."<input type=\"radio\" name=\"options[4]\" value=\"1\"";
    if ($options[4] == 1) $form .= " checked=\"checked\"";
    $form .= " />"._YES."<input type=\"radio\" name=\"options[4]\" value=\"0\"";
    if ($options[4] == 0) $form .= " checked=\"checked\"";
    $form .= " />"._NO;

    $form .= "<br />" . _MB_CBBX_TITLE_LENGTH."<input type='text' name='options[5]' value='" . $options[5] . "' />";

    $form .= "<br /><br />" . _MB_CBBX_FORUMLIST;

    $options_forum = array_filter(array_slice($options, 6), "b_cbbx_array_filter"); // get allowed forums
    ...
    $form .= ">"._ALL."</option>";
    $form .= cbbx_forumSelectBox($options_forum);
    /*
	$forum_handler =& xoops_getmodulehandler('forum', basename(dirname(__DIR__)));
...
function b_cbbx_topic_edit($options)
{
    $form  = _MB_CBBX_CRITERIA."<select name='options[0]'>";
    $form .= "<option value='time'";
	    if($options[0]=="time") $form .= " selected='selected' ";
...
function b_cbbx_author_edit($options)
{
    $form  = _MB_CBBX_CRITERIA."<select name='options[0]'>";
    $form .= "<option value='post'";
	    if($options[0]=="post") $form .= " selected='selected' ";
	    $form .= ">"._MB_CBBX_CRITERIA_POST."</option>";
    $form .= "<option value='topic'";
	    if($options[0]=="topic") $form .= " selected='selected' ";
	    $form .= ">"._MB_CBBX_CRITERIA_TOPIC."</option>";
    $form .= "<option value='digest'";
	    if($options[0]=="digest") $form .= " selected='selected' ";
	    $form .= ">"._MB_CBBX_CRITERIA_DIGESTS."</option>";
    $form .= "<option value='sticky'";
	    if($options[0]=="sticky") $form .= " selected='selected' ";
	    $form .= ">"._MB_CBBX_CRITERIA_STICKYS."</option>";
    $form .= "</select>";
    $form .= "<br />" . _MB_CBBX_DISPLAY."<input type='text' name='options[1]' value='" . $options[1] . "' />";
    $form .= "<br />" . _MB_CBBX_TIME."<input type='text' name='options[2]' value='" . $options[2] . "' />";
    $form .= "<br />&nbsp;&nbsp;&nbsp;&nbsp;<small>" . _MB_CBBX_TIME_DESC. "</small>";
    ...
    $form .= " />&nbsp;" . _MB_CBBX_DISPLAYMODE_COMPACT . "<input type='radio' name='options[3]' value='1'";
    if ($options[3] == 1) {
        $form .= " checked='checked'";
    }
    $form .= " />&nbsp;" . _MB_CBBX_DISPLAYMODE_LITE;
    ...
    $options_forum = array_filter(array_slice($options, 5), "b_cbbx_array_filter"); // get allowed forums
    $isAll = (count($options_forum)==0||empty($options_forum[0]))?true:false;
    $form .= "<br />&nbsp;&nbsp;<select name=\"options[]\" multiple=\"multiple\">";
    $form .= "<option value=\"0\" ";
    if ($isAll) $form .= " selected=\"selected\"";
