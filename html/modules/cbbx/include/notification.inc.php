...
if ( !defined('CBBX_NOTIFY_ITEMINFO') ) {
define('CBBX_NOTIFY_ITEMINFO', 1);

function cbbx_notify_iteminfo($category, $item_id)
{
	...
	if ($category=='forum') {
		...
		$sql = 'SELECT forum_name FROM ' . $xoopsDB->prefix('cbbx_forums') . ' WHERE forum_id = '.$item_id;
	...
	if ($category=='thread') {
		...
		$sql = 'SELECT t.topic_title,f.forum_id,f.forum_name FROM '.$xoopsDB->prefix('cbbx_topics') . ' t, ' . $xoopsDB->prefix('cbbx_forums') . ' f WHERE t.forum_id = f.forum_id AND t.topic_id = '. $item_id . ' limit 1';
		if (!$result = $xoopsDB->query($sql)){
			  redirect_header("index.php", 2, _MD_CBBX_ERROROCCURED);
    		exit();
		}
		$result_array = $xoopsDB->fetchArray($result);
	...
	if ($category=='post') {
		...
		$sql = 'SELECT subject,topic_id,forum_id FROM ' . $xoopsDB->prefix('cbbx_posts') . ' WHERE post_id = ' . $item_id . ' LIMIT 1';
		if (!$result = $xoopsDB->query($sql)){
			  redirect_header("index.php", 2, _MD_CBBX_ERROROCCURED);
    		exit();
		}
