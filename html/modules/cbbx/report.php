...
if ( empty($forum) ) {
    ...
    redirect_header("viewtopic.php?topic_id=$topic_id&amp;order=$order&amp;viewmode=$viewmode&amp;pid=$pid", 2, _MD_CBBX_ERRORPOST);
    exit();
}
...
if ( isset($_POST['submit']) ) {
	$report_handler =& xoops_getmodulehandler('report', basename(__DIR__));
	...
	$report_form->addElement(new XoopsFormText(_MD_CBBX_REPORT_TEXT, 'report_text', 80, 255), true);
	...
	$cancel_button = new XoopsFormButton('', 'cancel', _MD_CBBX_CANCELPOST, 'button');
    ...
    if( $xoopsModuleConfig['enable_karma'] && $forumpost->getVar('post_karma') > 0 ) {
        $r_message = sprintf(_MD_CBBX_KARMA_REQUIREMENT, "***", $forumpost->getVar('post_karma'))."</div>";
    }elseif( $xoopsModuleConfig['allow_require_reply'] && $forumpost->getVar('require_reply') ) {
        $r_message = _MD_CBBX_REPLY_REQUIREMENT;
    }else{
    ...
    $r_content = _MD_CBBX_SUBJECTC." ".$r_subject."<br />";
