<table class="outer" cellspacing="1">
  <{if $block.disp_mode == 0}>
    <tr>
      ...
      <th class="head" align="center" nowrap="nowrap"><{$smarty.const._MB_CBBX_LPOST}></th>
    </tr>
    <{foreach item=topic from=$block.topics}>
      <tr class="<{cycle values="even,odd"}>">
        <td><a href="<{$smarty.const.CBBX_URL}>/viewforum.php?forum=<{$topic.forum_id}>"><{$topic.forum_name}></a></td>
        <td><a href="<{$smarty.const.CBBX_URL}>/viewtopic.php?topic_id=<{$topic.id}>&amp;forum=<{$topic.forum_id}>&amp;post_id=<{$topic.post_id}>#forumpost<{$topic.post_id}>">
  ...
  <{elseif $block.disp_mode == 1}>
    ...
    <{foreach item=topic from=$block.topics}>
      <tr class="<{cycle values="even,odd"}>">
        <td><a href="<{$smarty.const.CBBX_URL}>/viewtopic.php?topic_id=<{$topic.id}>&amp;forum=<{$topic.forum_id}>&amp;post_id=<{$topic.post_id}>#forumpost<{$topic.post_id}>"><{$topic.title}></a></td>
        <td align="center"><{$topic.replies}></td>
        <td align="right"><{$topic.time}><br /><{$topic.topic_poster}></td>
      </tr>
    <{/foreach}>
...
<{if $block.indexNav}>
  ...
  <a href="<{$xoops_url}>/"><{$smarty.const._MB_CBBX_VSTFRMS}></a>
  </div>
<{/if}>